...
export function ExportPdfDialog({ stats, range, onClose }: Props) {
  ...
  // Synchronous double-click guard. setRunning(true) is batched by
  // React, so a fast second click can still slip through and start a
  // parallel pdf() render that doubles memory pressure and
  // overlapping Blob downloads. The ref flips synchronously inside
  // the click handler before the await suspends the microtask.
  ...
  useEffect(() => {
    ...
    // The `[tabindex]:not([tabindex="-1"])` clause is required so
    // future custom widgets (combobox, listbox) inside the dialog
    // participate in the trap instead of being skipped.
    const focusables =
      ...
      >
        <header className="flex items-start justify-between border-b border-surface-border px-5 py-4">
          ...
          <button
            ...
            type="button"
            onClick={onClose}
            className="grid min-h-[44px] min-w-[44px] place-items-center rounded-lg text-ink-secondary hover:bg-surface-alt hover:text-ink-primary"
            aria-label="Close"
          >
            <X className="h-5 w-5" />
          </button>
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
          <section className="space-y-3">
            ...
            <div>
              <label
                htmlFor="pdf-export-subtitle-input"
                className="block text-[10px] font-semibold uppercase tracking-wider text-ink-muted"
              >
                Subtitle
              </label>
              <input
                id="pdf-export-subtitle-input"
                type="text"
                value={subtitle}
        ...
        <footer className="flex items-center justify-end gap-2 border-t border-surface-border bg-surface-alt px-5 py-3">
          <button
            ...
            onClick={onClose}
            disabled={running}
            className="min-h-[44px] rounded-pill px-4 py-2 text-xs font-medium text-ink-secondary hover:text-ink-primary"
          >
            Cancel
          </button>
          <button
            ...
            className="inline-flex min-h-[44px] items-center gap-1.5 rounded-pill bg-primary px-4 py-2 text-xs font-semibold text-white shadow-card transition-colors hover:bg-primary-dark disabled:cursor-not-allowed disabled:opacity-50"
          >
